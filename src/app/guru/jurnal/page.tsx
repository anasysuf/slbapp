            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {journals.map((j) => {
                const isFromParent =
                  ...
                  (j.authorId && j.authorId === j.student?.parentId) ||
                ...
                const authorDisplayName =
                  j.authorName ||
                  j.author?.name ||
                  (isFromParent ? j.student?.parent?.name || "Orang Tua Siswa" : j.teacher?.name || "Guru Kelas");

                return (
                  <div
                    key={j.id}
                    className={`bg-white rounded-3xl border shadow-sm p-5 space-y-3 flex flex-col justify-between transition-all ${
                      isFromParent
                        ? "border-emerald-300/90 bg-emerald-50/20 ring-1 ring-emerald-200"
                        : isFromAdmin
                    ...
                    <div className="space-y-3">
                      ...
                      <div className={`p-3 rounded-xl border text-xs leading-relaxed ${
                        isFromParent
                          ...
                          : "bg-teal-50/40 border-teal-100 text-slate-700"
                      }`}>
                        <span className="font-bold block mb-1">
                          {isFromParent ? "🏡 Kabar & Aktivitas dari Rumah:" : "🏫 Aktivitas Terapi & Pembelajaran di Sekolah:"}
                        </span>
                        {j.learningActivity}
                      </div>
                      ...
                      {j.parentFeedback ? (
                        ...
                        <div className="text-[11px] text-slate-400 italic">
                          {isFromParent ? "Catatan ini dikirim langsung oleh orang tua murid." : "Menunggu konfirmasi / catatan respon dari orang tua di rumah."}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="pt-3 border-t border-slate-100 text-[11px] text-slate-500 flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-1.5 font-medium">
                        ...
                        <span className={`text-[10px] font-bold px-2 py-0.2 rounded-md ${
                          ...
                          {isFromParent ? "Orang Tua / Wali" : isFromAdmin ? "Admin" : "Guru Kelas"}
                        </span>
                      </div>
                    </div>
                  </div>
